...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/02 09:30:16 by mhirvasm          #+#    #+#             */
/*   Updated: 2025/10/02 09:41:04 by mhirvasm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @return pointer to an array of (cmd_count - 1) pipe pairs, or 
 * 												NULL if none/failed.
 *
...
int	(*allocate_pipes(int cmd_count))[2]
{
	...
	if (cmd_count <= 1)
		return (NULL);
	pipe_pairs = (int (*)[2])malloc(sizeof(int [2]) * (cmd_count - 1));
	if (!pipe_pairs)
		return (NULL);
	pipe_index = 0;
